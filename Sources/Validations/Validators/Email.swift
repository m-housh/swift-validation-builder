...
public extension Validator where Value == String {
  ...
  ///
  @inlinable
  static func email(_ style: Validators.EmailValidator<Self>.Style = .default) -> Self {
    .init(Validators.EmailValidator<Self>(style: style))
  }
}

public extension AsyncValidator where Value == String {
...
public extension Validators {
  ...
  /// ```
  ///
  struct EmailValidator<ValidationType> {
  ...
  public var body: some Validation<String> {
    Validator<Value> {
      ...
      Validator.accumulating { // accumulate errors if not.
        ...
        // length before the @
        // Validator.mapValue<Int> { (string: String) -> Int in
        //   string.split(separator: "@")[0].count
        // } with: {
...
private extension Validators.EmailValidator.Style {
