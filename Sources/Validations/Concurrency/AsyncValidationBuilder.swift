...
public enum AsyncValidationBuilder<Value> {
  ...
  @inlinable
  public static func buildBlock<V: AsyncValidator>(_ components: V) -> _SequenceMany<V> where V.Value == Value {
    _SequenceMany([components])
  }
  
  @inlinable
  public static func buildArray<V: AsyncValidator>(_ components: [V]) -> _SequenceMany<V> where V.Value == Value {
    _SequenceMany(components)
  }
  ...
  @inlinable
  public static func buildExpression<V: AsyncValidator>(_ expression: V) -> V where V.Value == Value {
    expression
  }
  
  @inlinable
  public static func buildFinalResult<V: AsyncValidator>(_ component: V) -> V where V.Value == Value {
    component
  }
  
  public struct _Sequence<V0: AsyncValidator, V1: AsyncValidator>: AsyncValidator where V0.Value == V1.Value {
